...
def read_existing_silver(bucket_name, table_name):
    ...
    try:
        silver_path = f"s3://{bucket_name}/{table_name}.parquet"
        
        print(f"📖 Intentando leer Silver existente: {silver_path}")
...
for table in tables:
    ...
    try:
        ...
        # 8. Guardar Silver (sobrescribir)
        if total_final_records > 0:
            silver_s3_path = f"s3://{SILVER_S3_BUCKET}/{table}.parquet"
            
            try:
                df_final.coalesce(1).write.mode('overwrite').option('compression', 'snappy').parquet(silver_s3_path)
                print(f"✅ Silver guardado: {silver_s3_path}")
                ...
                print(f"❌ Error guardando Silver en S3: {e}")
        else:
